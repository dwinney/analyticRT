...
#ifndef UNITARY_V2_HPP
#define UNITARY_V2_HPP
...
{
    class unitary_v2 : public raw_trajectory
    {
        public:
        ...
        inline double RHC(double s)
        {
            ...
            if (s > 100) return gamma()*(sqrt(s)*log(q2hat()) + log(beta()/gamma()));
            
            double exponent = (RePart() > _jmin) ? RePart() : _jmin;
            return gamma()* log(1. + rho()*beta()/gamma()*(1.+ pow(q2hat(), exponent)));
        };
