...
void replot()
{
    ...
    auto jrp_curves = import_data<5>(dir + "alpha-rho-error-plb-central.dat");


    data_set rhos = rho_spectrum();
    data_set as   = a_spectrum();

    auto img = [&](double s, double gamma)
    {
        auto f = [&](double x){ return alpha->real_part(x); };
        double alphaPrime = boost::math::differentiation::finite_difference_derivative(f, s);
        return  alphaPrime*sqrt(s)*gamma;
    };
    ...
    p1.add_dashed( jrp_curves[0], jrp_curves[1]);
    p1.add_curve(  {-0.2, 1.5},      [alpha](double s){ return alpha->imaginary_part(s); }, "Imaginary");
    p1.add_dashed( jrp_curves[0], jrp_curves[2]);
    p1.add_curve(  {-0.2, 1.5},      [alpha](double s){ return 0.5+0.9*s; }, dotted(jpacColor::DarkGrey, "0.5 + 0.9 #it{s}"));
    p1.add_data({square_elementwise(rhos._x), {}}, {rhos._y, {}}, jpacColor::DarkGrey);
    p1.add_data({square_elementwise(rhos._x), {}}, {std::vector<double>{img(rhos._x[0]*rhos._x[0], 0.148)}, {}}, jpacColor::DarkGrey);
    ...
    p2.add_curve( {STH + EPS, 1}, [](double s){ return std::real(pipi::partial_wave(1, 1, s));}, dotted(jpacColor::DarkGrey, "GKPY"));
    p2.add_curve( {STH + EPS, 1}, [](double s){ return std::imag(pipi::partial_wave(1, 1, s));}, dotted(jpacColor::DarkGrey));
    p2.save("f11.pdf");
    ...
    p3.add_curve(  {-1.5, 0},      [alpha](double s){ return 0.5+0.9*s; }, dotted(jpacColor::DarkGrey));
